...
--
-- @tparam string args.uri                      MQTT broker uri to connect. Expecting "host:port" or "host" format, in second case the port will be selected automatically: 1883 port for plain or 8883 for secure network connections
--
-- @tparam boolean args.clean                   clean session start flag
--
-- @tparam[opt=4] number args.version           MQTT protocol version to use, either 4 (for MQTT v3.1.1) or 5 (for MQTT v5.0). Also you may use special values mqtt.v311 or mqtt.v50 for this field.
--
...
--
-- @tparam[opt=false] boolean,table args.secure use secure network connection; set to true to select default params or set to table, for example with certificate="...", key="..."
--
-- @tparam[opt] table args.will                 will message table with required fields { topic="...", payload="..." } and optional fields { qos=1...3, retain=true/false }
--
...
function client_mt:__init(args)
    ...
    for key, value in pairs(args) do
        ...
        elseif key == "ping_limit" then
            assert(value_type == "number", "expecting ping_limit to be a number")
            a.ping_limit = value
        else
...
function client_mt:send_pingreq()
    -- check connection is alive
    if self._ping_count < self.args.ping_limit then
        if not self.connection then
            return false, "network connection is not opened"
        end
        ...
        self._ping_count = self._ping_count + 1
        return true
    else
        self._ping_count = 0
        self:handle("pinglimit", self._ping_count, self)
        return false
    end
end
...
-- Perform loop input/output iteration
function client_mt:iteration()
    return self:_io_iteration()
end

-- Performing one IO iteration - receive next packet
function client_mt:_io_iteration()
    local conn = self.connection
...
function client_mt:_send_packet(packet)
    ...
    local ok = send(conn, data)
    if not ok then
        return false, "connector.send failed"
    end
